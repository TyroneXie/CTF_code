...
class Predictor(VarScopeObject):
    ...
    def get_score(self, values, sample_ratio=1.0, mode='train'):
        """
        ...
        with tf.name_scope('Predictor.get_score'):
            ...
            if mode == 'train':
                ...
                values = values.reshape(-1, 100, 49)
                for i in range(int(values.shape[0] / self._batch_size) + 1):
                    start_iter_time = time.time()
                    feed_dict = dict(six.iteritems(self._feed_dict))
                    feed_dict[self._input_x] = tuple(
                        values[j, :, :] for j in range(
                            int(i * self._batch_size), min(int((i + 1) * self._batch_size), values.shape[0])
                        )
                    )
                    b_r, q_net_z = sess.run(self._get_score_without_y(),
                                            feed_dict=feed_dict)
